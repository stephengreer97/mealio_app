...
function buildSearchAndAddScript(
  ...
  for (var pi = 0; pi < productLinks.length && pi < 20; pi++) {
    ...
    // which in _scoring.ts means equality after normalization and critical-phrase
    // collapse and nothing else, because every path but the na === nb fast path is
    // capped by Math.min(99, …). The collapse half is MEAL-160: "lowfat milk" and
    // "Low Fat Milk" are one product spelled two ways, and the gate now treats
    // them as such. Albertsons compares normalized strings directly. ALDI was the
    // exception.
    //
