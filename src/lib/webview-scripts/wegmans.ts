...
function buildSearchAndAddScript(
  ...
  // MEAL-160: two-words-on-one-label, one-word-on-the-next. The veto is a token
  // membership test, so 'lowfat' as a lone entry both failed to fire for the
  // natural query spelling ("low fat" tokenises to two non-critical words) and
  // zeroed the RIGHT product when it did fire (no hyphenated label can contain
  // it). Collapsed on both sides so the entry means the concept, not one
  // spelling of it. Longest first — 'cage free' and 'free range' share a word.
  // Byte-for-byte the same rule as CRITICAL_PHRASES in _scoring.ts.
