...
(async () => {
  ...
  const page = b.contexts()[0].pages().find((p) => /amazon\.com/.test(p.url()));
  if (!page) { console.log('no amazon page'); process.exit(1); }
  await page.addInitScript(RECORDER);
  const term = process.env.AMZ_TERM || 'sour cream';
  await page.goto('https://www.amazon.com/s?k=' + encodeURIComponent(term) + '&i=amazonfresh',
  ...
  const clicked = await page.evaluate(`(function () {
    ...
    for (var i = 0; i < all.length; i++) {
      ...
      if (/add to cart/i.test(al) || /^Add to cart$/i.test(tx) || /add-to-cart|addToCart/i.test(al)) {
